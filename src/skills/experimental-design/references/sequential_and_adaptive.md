...
The adapted bundled dependency set does not provide a confirmatory
group-sequential boundary package and never launches R, `rpy2`, or an external
subprocess. Use an already-authorized domain tool only when the dispatch names
it and the environment/evidence record pins it; otherwise return blocked for a
qualified statistician/tool decision. For a custom exploratory rule, simulate
the complete procedure with the approved project environment and verify realized
Type I error, expected sample size, and power through `statistical-power`.
