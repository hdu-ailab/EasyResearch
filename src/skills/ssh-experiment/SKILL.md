...
name: ssh-experiment
description: |-
  Run GPU-intensive deep learning experiments on SSH server hz110-cyy (3x RTX3090-24G).
  Check GPU availability via nvidia-smi before launching, select free GPUs with CUDA_VISIBLE_DEVICES,
  mount ServerHome via sshfs if needed. Use proactively when user needs image recognition,
  deep learning, or any task requiring GPU acceleration.
  ...
  - user: "Mount server home" → sshfs cyy@hz110-cyy: ~/Papers/ServerHome
---
...
# Mount if ServerHome is empty
sshfs cyy@hz110-cyy: ~/Papers/ServerHome
```

Edit code, check logs — use Read/Edit/Write tools on `~/Papers/ServerHome/workspace/experiments/` (no SSH).

**All SSH to hz110-cyy MUST use `rssh`** (`.opencode/skills/ssh-experiment/scripts/rssh`). This wrapper adds keepalive (`ServerAliveInterval=30`, `ServerAliveCountMax=10`) and timeout handling to prevent connection drops. Use `-f` to launch background jobs that return immediately.

For interactive sessions, use `rssh-tmux`.

## Mount Check

Before any server operation, verify `~/Papers/ServerHome/` is mounted — use Read tool on the directory. If it returns empty or only `.` and `..`, mount it:

```bash
sshfs cyy@hz110-cyy: ~/Papers/ServerHome
```

The ServerHome is a direct SSHFS mirror of the remote home directory. Files written here appear on the server and vice versa.

## Local vs Remote: Core Principle

**ServerHome is sshfs-mounted** → all files are accessible locally. Only TWO things go through SSH:

| Action | Where | How |
|--------|-------|-----|
| Edit code, write files | **Local** | Use Edit/Write tools on `~/Papers/ServerHome/workspace/experiments/src/` |
| Read logs, check outputs | **Local** | Use Read tool on `~/Papers/ServerHome/workspace/experiments/logs/` |
...
Use Edit/Write tools on `~/Papers/ServerHome/workspace/experiments/src/` — mirrored to server instantly via sshfs.
...
2. **Read log locally** — use Read tool or `cat` on the mounted path. NO SSH needed:
   ```
   cat ~/Papers/ServerHome/workspace/.../logs/<run-id>.log
   ```
