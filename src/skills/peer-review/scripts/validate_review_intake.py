...
def validate_intake(payload: Any) -> dict[str, Any]:
    ...
    if external_service_use:
        if local_only:
            errors.append(issue("HANDLING_MODE_CONFLICT", "handling.local_only"))
        ...
        if ai_plan != "approved_ai_assistance":
            errors.append(issue("EXTERNAL_AI_PLAN_REQUIRED", "ai_use.planned"))
    elif not local_only:
        errors.append(issue("LOCAL_HANDLING_NOT_DECLARED", "handling.local_only"))
    elif external_authorized:
        warnings.append(
            issue(
    ...
    return {
        ...
        "review_id": review_id,
        "valid": valid,
        "status": "READY_FOR_DECLARED_REVIEW" if valid else "BLOCKED",
        "errors": errors,
        "warnings": warnings,
        "normalized_scope": {
            "capacity": capacity,
        ...
        "handling_assertions": {
            "bundled_tools_are_local_only": not external_service_use,
            "external_service_use_declared_authorized": bool(
                valid and external_service_use and external_authorized
            ),
            "data_reuse_authorized": False,
            "source_artifacts_must_not_be_modified": True,
        },
        "notice": (
            "This validates declared process controls only. It does not establish "
            "reviewer competence, resolve conflicts, grant processing authority, or "
            "assess manuscript quality. External readiness reflects only the "
            "authorization and policy declarations supplied to this validator."
        ),
    }
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Validate an authorized peer-review intake JSON without reading or echoing "
            "manuscript content."
        )
    )
