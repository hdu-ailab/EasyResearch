...
Run read-only tools from the Skill directory or use absolute script paths. Keep
the exact paper-project root as the working directory for any tool that writes a
project artifact, and invoke that tool through its absolute Skill script path.
...
- Require either declared local-only handling or explicitly authorized
  configured-provider processing; prohibit data reuse and require a
  retention/deletion decision record
...
```bash
"$EASYRESEARCH_VENV/bin/python" "<peer-review-skill-directory>/scripts/generate_review_scaffold.py" \
  completed-intake.json \
  -o reviews/.draft-review_report-<UUID>.md
```

Run this command from the exact paper-project root; replace the bracketed Skill
directory with the absolute directory of the loaded `peer-review` Skill.
...
- Provides one internal ADR-102 report with structured major/minor findings
- Requires exact evidence locators, required actions, and owning Agents
...
```bash
"$EASYRESEARCH_VENV/bin/python" "<peer-review-skill-directory>/scripts/lint_review.py" reviews/.draft-review_report-<UUID>.md
```
...
- Missing/duplicate Review Report heading
...
Final Review reports never use `--force`; publish the completed hidden draft
through `specialist-handoff/scripts/publish_immutable.py`. Review does not delete
inputs, evidence, drafts owned by another specialist, or prior reports. Record a
retention/deletion requirement for Research Assistant/user routing.
