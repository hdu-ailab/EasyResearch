...
Requires: numpy, scipy, statsmodels; mixed-model examples also use pandas.
"""
...
def simulate_power(gen_and_test, n, n_sims=2000, seed=0):
    ...
    gen_and_test(n, rng) -> bool : build a dataset of size n under the assumed
        ...
        replicate is independent. The function closes over its declared alpha;
        see the examples.
...
def find_sample_size(gen_and_test, target_power=0.80, n_sims=2000,
    ...
    while True:
        est_hi = simulate_power(gen_and_test, hi, n_sims, seed)
    ...
    while lo < hi:
        mid = (lo + hi) // 2
        est = simulate_power(gen_and_test, mid, n_sims, seed)
        if verbose:
            print(est)
