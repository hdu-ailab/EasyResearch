...
def _corr_power(r, n, alpha, alternative):
    ...
    mean = math.atanh(r) * math.sqrt(n - 3)
    if alternative == "two-sided":
        zc = stats.norm.ppf(1 - alpha / 2)
        return float(stats.norm.cdf(mean - zc) + stats.norm.cdf(-mean - zc))
    zc = stats.norm.ppf(1 - alpha)
    if alternative == "larger":
        return float(stats.norm.cdf(mean - zc))
...
def _corr_sample_size(r, alpha, power, alternative):
    from scipy import stats
    if alternative == "larger" and r <= 0:
        raise ValueError("correlation alternative='larger' requires a positive effect_size")
    if alternative == "smaller" and r >= 0:
        raise ValueError("correlation alternative='smaller' requires a negative effect_size")
    z = abs(math.atanh(r))
    if alternative == "two-sided":
        zc = stats.norm.ppf(1 - alpha / 2)
    else:
...
def _reg_sample_size(f2, df_num, k_total, alpha, power):
    if _reg_power(f2, 1_000_000, df_num, k_total, alpha) < power:
...
def mde(test, nobs1=None, nobs=None, alpha=0.05, power=0.80,
    ...
    if solver == "correlation":
        ...
        effect = float(math.tanh(z))
        return -effect if alternative == "smaller" else effect
