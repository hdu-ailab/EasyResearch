...
Use only the public shared pool and expect 429/partial availability. Never add an
`x-api-key` header or read a key. After one bounded retry, record the failure and
use another public index or return a partial handoff.
