...
on a verified public publisher/repository source page rather than presenting
`<front>` metadata as the article.
...
| `<error code="idIsNotOpenAccess">` | The article exists but is **not** in the OA Subset -- eFetch will return metadata only. Route to Europe PMC or a verified public publisher/repository page. |
...
| Service | Limit |
|---------|-------|
| E-utilities (`db=pmc`) | 3/sec public unauthenticated |
| BioC API | 3/sec public unauthenticated |
| ID Converter | Follow general NCBI policy |

Use only public unauthenticated requests. Large batch jobs should run outside
peak hours (Mon-Fri 5AM-9PM ET).
