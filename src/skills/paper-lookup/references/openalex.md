...
The EasyResearch adaptation uses only public unauthenticated requests. Never add
an API key, `mailto`, or another credential/contact query parameter. If the
public allowance is insufficient, return a partial/blocked handoff.
...
Abstracts are stored as `{word: [positions]}`. Use the bundled
`scripts/openalex_abstract.py`; it preserves duplicate positions and validates
malformed indexes. Do not reimplement reconstruction from this response sketch.
...
HTTP 403/429 may indicate denied public access or rate limiting. Error responses
include a message field; report the failure without adding credentials.
