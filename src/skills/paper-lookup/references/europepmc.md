...
```text
scripts/paginate.py --api europepmc --query 'CRISPR AND SRC:"PPR"' --max-records 2
```

Invoke the helper with the platform-specific Python command from `SKILL.md`;
do not construct a shell request manually.
...
```text
scripts/paginate.py --api europepmc --query '(SRC:"PPR" AND PUBLISHER:"bioRxiv" AND "organoid")' --max-records 2
```
