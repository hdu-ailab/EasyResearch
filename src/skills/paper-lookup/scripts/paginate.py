...
Needs network access and uses public unauthenticated bioRxiv, medRxiv, Europe
PMC, Crossref, and OpenAlex routes. Credential/contact query parameters are
rejected.
"""
...
from _common import Reconciliation, emit, fail, is_sensitive_parameter, redact_url  # noqa: E402
    ...
    try:
        with urllib.request.urlopen(request, timeout=REQUEST_TIMEOUT) as response:
            ...
            if len(raw) > MAX_RESPONSE_BYTES:
                raise RuntimeError(
                    f"response exceeded {MAX_RESPONSE_BYTES} bytes from {redact_url(url)}"
                )
            body = raw.decode("utf-8", errors="replace")
    except urllib.error.HTTPError as error:
        detail = error.read(401).decode("utf-8", errors="replace")[:400]
        raise RuntimeError(
            f"HTTP {error.code} from {redact_url(url)}: {detail}"
        ) from error
    except urllib.error.URLError as error:
        raise RuntimeError(
            f"could not reach {redact_url(url)}: {error.reason}"
        ) from error
    try:
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not 1 <= args.max_records <= DEFAULT_MAX_RECORDS:
        fail(f"--max-records must be between 1 and {DEFAULT_MAX_RECORDS}")
    if not 1 <= args.max_calls <= DEFAULT_MAX_CALLS:
        fail(f"--max-calls must be between 1 and {DEFAULT_MAX_CALLS}")
    for name, _value in urllib.parse.parse_qsl(args.query, keep_blank_values=True):
        if is_sensitive_parameter(name):
            fail(f"credential/contact query parameter is not supported: {name.lower()}")
