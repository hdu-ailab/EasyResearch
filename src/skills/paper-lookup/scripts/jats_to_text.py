...
metadata, so the caller can fall back to Europe PMC (a clean 404), a verified
public publisher/repository page, or the abstract without claiming full text.

Handles both wrappers: eFetch's `<pmc-articleset><article>` and Europe PMC's bare
`<article>`.

    Save a public JATS response, then run: python3 jats_to_text.py article.xml
    ...
    if body is None:
        ...
        payload = {
            ...
            "guidance": (
                ...
                "check the PMC OA Web Service for a downloadable package, or verify a public "
                "publisher/repository source page."
            ),
        }
        if comments:
