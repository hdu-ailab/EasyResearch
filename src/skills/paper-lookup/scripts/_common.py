    ...
    def as_dict(self) -> dict[str, Any]:
        ...
        if self.expected is None:
            ...
            summary["shortfall_reason"] = (
                "bounded by --max-records/--max-calls; narrow or partition the query to continue"
                if self.stopped_at_limit
...
def is_sensitive_parameter(name: str) -> bool:
    """Return whether a query parameter can carry credentials or contact data."""
    normalized = re.sub(r"[^a-z0-9]+", "_", name.casefold()).strip("_")
    compact = normalized.replace("_", "")
    return (
        normalized in REDACTED_PARAMS
        or normalized.endswith(
            ("_api_key", "_credential", "_credentials", "_password", "_passwd", "_secret", "_token")
        )
        or compact.endswith(("apikey", "credential", "credentials", "password", "passwd", "secret", "token"))
    )
...
def redact_url(url: str) -> str:
    ...
    if not parts.query:
        return url
    pairs = [
        (name, REDACTION if is_sensitive_parameter(name) else value)
