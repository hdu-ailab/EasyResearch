...
describe("ConfigBrowser", () => {
  ...
  it("lists files under the global scope only", async () => {
    render(<ConfigBrowser />);
    expect(await screen.findByText("settings.json")).toBeTruthy();
    expect(api.listConfig).toHaveBeenCalledWith("global", undefined, undefined);
  });

  it("never offers a project scope", () => {
    render(<ConfigBrowser />);
    expect(screen.queryByRole("tab", { name: /project/i })).toBeNull();
    expect(screen.queryByRole("tab", { name: /global/i })).toBeNull();
  });
  ...
  it("navigates into directories and back up", async () => {
    ...
    await waitFor(() => expect(api.listConfig).toHaveBeenLastCalledWith("global", undefined, "sub"));
  ...
  it("opens a file into the editor", async () => {
    ...
    expect(api.readConfigFile).toHaveBeenCalledWith("global", undefined, "settings.json");
  ...
  it("saves valid JSON via writeConfigFile without restarting the session", async () => {
    ...
    await waitFor(() => expect(api.writeConfigFile).toHaveBeenCalledWith("global", undefined, "settings.json", '{"b":2}'));
  ...
  it("creates a directory via the folder action", async () => {
    ...
    await waitFor(() => expect(api.createConfigDirectory).toHaveBeenCalledWith("global", undefined, "models"));
  });
  ...
  it("keeps secret-bearing auth.json content only in the editor value and API call", async () => {
    ...
    await user.click(screen.getByRole("button", { name: /save/i }));
    await waitFor(() =>
      expect(api.writeConfigFile).toHaveBeenCalledWith("global", undefined, "auth.json", '{"apiKey":"sk-secret"}'),
    );
    expect(screen.queryByText(/sk-secret/, { selector: "button, .v2-md" })).toBeNull();
  });
